...
def train_model(rank, args):
    ...
    for epoch in range(args.n_epochs):
        train_data_sampler = torch.utils.data.distributed.DistributedSampler(dataset=training_set,
                                                                         num_replicas=args.world_size, rank=rank)
        print("test1")
        trainLoader = torch.utils.data.DataLoader(dataset=training_set, batch_size=batch_size,
                                              shuffle=False, num_workers=0, pin_memory=True,
                                              sampler=train_data_sampler)
        print("test2")
    # Preparing the testing data
        ...
        test_data_sampler = torch.utils.data.distributed.DistributedSampler(dataset=testing_set,
                                                                        num_replicas=args.world_size, rank=rank)
        testLoader = torch.utils.data.DataLoader(dataset=testing_set, batch_size = batch_size,
                                             shuffle = False, num_workers=0, pin_memory=True,
                                             sampler=test_data_sampler)
        model.train()
        train_loss = 0
        ...
        losses = [torch.tensor([0.0]).to(rank) for _ in range(world_size)]
        torch.distributed.all_gather(losses, torch.tensor([train_loss]).to(rank))
        avg_loss = sum(losses) / world_size
        new_batch_size = trainloader.batch_size
        if avg_loss > target_loss:
            new_batch_size //= 2  # Decrease batch size if loss is higher than target
        else:
            new_batch_size *= 2  # Increase batch size if loss is lower than target
