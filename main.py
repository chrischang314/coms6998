...
def adjust_batch_size(prev_combined_score, rank, loss, train_time, world_size, batch_size):
    tolerance = 10
    if rank == 0:
    ...
    train_times = [torch.tensor([0.0]).cuda() for _ in range(4)]
    torch.distributed.all_gather(train_times, torch.tensor([train_time]).cuda())
    losses = [torch.tensor([0.0]).cuda() for _ in range(4)]
    torch.distributed.all_gather(losses, torch.tensor([loss]).cuda())
    avg_loss = sum(losses) / 4
    avg_train_time = sum(train_times) / 4

    combined_score = loss_weight * avg_loss + time_weight * avg_train_time
    if rank == 0:
        print(f"Combined score: {combined_score}")

    # Adjust batch size based on the combined score
    if combined_score > prev_combined_score + tolerance:
        batch_size = max(batch_size // 2, s_min)
    elif combined_score < prev_combined_score - tolerance:
        batch_size = min(batch_size * 2, s_max)

    prev_combined_score = combined_score

    new_batch_size = torch.tensor([batch_size]).cuda()
    torch.distributed.broadcast(new_batch_size, src=0)
    batch_size = new_batch_size.item()
...
def train_model(rank, args):
    ...
    for epoch in range(args.n_epochs):
        ...
        new_batch_size = adjust_batch_size(prev_combined_score, train_loss, prev_train_losses, train_time, args.world_size, trainloader.batch_size)
